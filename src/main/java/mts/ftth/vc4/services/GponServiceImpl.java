...
public class GponServiceImpl implements GponService {
    ...
    @Override
    public ResponseEntity<APIResponse> getGPONNumbersList(String authToken, int nodeId) {
        try {

            String url = "https://10.98.35.78:2080/api/ims/CustomDataOperation/ExecuteProcedure/Get_GPON_Numbers_List";

            Map<String, Object> body = new HashMap<>();
            body.put("NODE_ID", nodeId);

            GenericRestClient<GPONNumbersListResponse, Map<String, Object>> genericRestClient = new GenericRestClient<>();
            GPONNumbersListResponse gponNumbersListResponse1 = genericRestClient.post(url, GPONNumbersListResponse.class, body, authToken, GenericRestClient.AuthType.BEARER);
            //	gponNumbersListResponse1.getParameters().getOntList().stream().forEach(ont -> System.out.println(ont.getCardPort()));;
//			assert gponNumbersListResponse != null;
//			gponNumbersListResponse.getParameters().getOntList().stream().forEach(ont -> System.out.println(ont.getOntId()));
            ...
            apiResponse.setClientMessage("Error creating GPON number List request");
